...
# find vector/scalar files for a rebalancing experiment
function process_single_rebalancing_experiment_results {
    prefix=$1
    balance=$2
    num=$3
    routing_scheme=$4
    ...
    graph_op_prefix=${GRAPH_PATH}/${prefix}_dag${dag_amt}_${balance}_num${num}_delay${delay}_demand${demand_scale}0
    graph_name=${graph_op_prefix}${routing_scheme}_${path_choice}_${num_paths}_${scheduling_alg}
    graph_name=${graph_name}_rebalancing${rebalancing_rate}
