...
(See Figure 1 of \url{https://arxiv.org/pdf/1709.05748.pdf}). 
...
\begin{itemize}
    ...
    \item ClearAmtMsg
        \begin{itemize}
            ...
            \item Landmark $i$
            \item Destination
            \item Clearing amount $c_i$ for this path
            \item Path to clear amount on
        \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \item When a successful routeDiscoverMsg has been received for landmark $i$, send the ``clear amount 
        $c_i$`` on that path message
    \item If a failure was received instead, send out a timeoutMsg (on that path $i$) alone to release funds
        that were reserved (this path will be reflected in reverse in the failure message, so reuse that)
...
\begin{itemize}
    ...
    \item If failure is false, 
        \begin{itemize}
            \item find the closest neighbor based on coordinate distances and as long as the balance on the payment 
        channel between current hop and neighbor is atleast $c_i$
            \item If such a neighbor is found, append this neighbor to the route and forward RouteDiscoverMsg 
        onto the next router
            \item Reserve funds $c_i$ on the payment channel between current node and neighbor or in other 
                words add it to the inflight funds on this payment channel at the current node's end.
            \item If no such neighbor found, set Failure to true, reverse the route and 
                send it back to the sender
        \end{itemize}
    \item If failure is true, just forward the message back all the way to sender
\end{itemize}
