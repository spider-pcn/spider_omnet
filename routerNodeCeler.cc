...
void routerNodeCeler::celerProcessTransactions(int endLinkNode){
    if (endLinkNode != -1){
        ...
        for(auto iter = nodeToPaymentChannel.begin(); iter != nodeToPaymentChannel.end(); ++iter)
        {
            ...
            //KATHY - TODO: Question: processTransUnits might not use up all the balance on the payment channel,
