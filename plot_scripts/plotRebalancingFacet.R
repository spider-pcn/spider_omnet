...
succ_ratio_plot <- ggplot(
                        ...
                        aes_string(x="RebalancingRate",y="SuccRatio", colour="Scheme", 
...
succ_volume_plot <- ggplot(
                        data[data$Topo != "lnd",],
                        aes_string(x="RebalancingRate",y="SuccVolume", colour="Scheme", shape="Scheme", 
                                   linetype="Scheme")) +
        geom_line(size=1) +
        facet_wrap( ~ Topo, labeller=labeller_func) +
        geom_point(size=3) +
        ...
        theme(legend.text=element_text(size=rel(0.9)), legend.key.size=unit(12,"points"), legend.position="none")
